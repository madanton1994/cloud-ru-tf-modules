resource "huaweicloud_lb_loadbalancer" "nlb" {
  # depends_on            = [huaweicloud_cce_node_pool.k8s_node_pool]
...
#   depends_on    = [huaweicloud_lb_loadbalancer.nlb]
...
#   # loadbalancer_id           = huaweicloud_lb_loadbalancer.nlb["${var.project}.${var.environment}.${each.value.nlb_index_name}"].id
#   loadbalancer_id           = "123"
...
# resource "huaweicloud_lb_pool" "target_group" {
#   depends_on  = [huaweicloud_lb_listener.listener, huaweicloud_lb_loadbalancer.nlb]
...
#   listener_id = huaweicloud_lb_listener.listener["${var.project}.${var.environment}.${each.value.nlb_index_name}.${each.value.listener_id_name}"].id
#   dynamic "persistence" {
...
# data "huaweicloud_cce_node_pool" "node_ingress" {
#   for_each   = { for ingrress_node_pool in data.huaweicloud_cce_cluster.cluster : "${ingrress_node_pool.name}" => ingrress_node_pool }
#   cluster_id = each.value["claims-cce-cluster"].cluster_id
#   # name       = each.value.name.
# }

# resource "huaweicloud_cce_addon" "addon" {
